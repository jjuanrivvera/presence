#!/bin/sh
# Claude Code SessionEnd hook: remove this session from plexus.
...
# Tear down this session's web terminal (ttyd), if any — now via `plexus ttyd kill`.
[ -n "${CLAUDE_SESSION_ID:-}" ] && "$BIN" ttyd kill "$CLAUDE_SESSION_ID" 2>/dev/null
[ -n "${CLAUDE_SESSION_ID:-}" ] && rm -f "$HOME/.local/state/plexus/sessions/$CLAUDE_SESSION_ID" 2>/dev/null
exit 0
