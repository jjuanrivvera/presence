...
name: plexus
description: >-
  Operate the plexus "plexus" — a cross-machine registry, launcher, and web
  ...
  "kill the plexus session", "the cockpit", "plexus list/get".
---

# plexus

`plexus` (a symlink to the `plexus` binary) is one tool for a fleet of
...
3. **Cockpit** — a web/PWA dashboard (`$PLEXUS_URL/ui`) that lists the fleet
   and embeds each session's live terminal (view + type + interrupt).

## Setup

The client reads config from flags → env vars → `~/.config/plexus/env`:

```
PLEXUS_URL=http://<registry-host>:<port>   # the plexus server
PLEXUS_TOKEN=<shared-secret>               # bearer + web login + terminal auth
PLEXUS_HOST=<label>                        # this machine's label (e.g. laptop)
```

Check it works: `plexus ls`. If it errors on `PLEXUS_URL`/`PLEXUS_TOKEN`,
those aren't set.
...
Open `$PLEXUS_URL/ui` in a browser (installable as a PWA). Log in once with
`PLEXUS_TOKEN`. Click a session in the sidebar to see its live terminal on the
...
- `plexus get --repo <R> [--host a,b] [--agent codex] -o json` — the freshest
...
- **Start or resume work in a repo** → `plexus claude <dir>` (reattaches if it's
  already open there).
- **Hand off / delegate** → `plexus get --repo <R>` to find the target
